...
app.get("/health", (req, res) => res.json({ ok: true, app: "boda-diego-meylin-v3" }));
...
app.post("/api/upload-chunk", requireGuestApi, express.raw({ type: "application/octet-stream", limit: "9mb" }), async (req, res) => {
  try {
    ...
    writeDB(db);
    await fsp.rm(dir, { recursive: true, force: true });
    res.json({ ok: true, complete: true, file: { id: item.id, token: item.publicToken } });
  } catch (err) {
    console.error(err);
...
app.listen(PORT, "127.0.0.1", () => {
  console.log("Diego & Meylin V3 en http://127.0.0.1:" + PORT);
});
